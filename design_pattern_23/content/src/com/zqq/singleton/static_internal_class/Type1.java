package com.zqq.singleton.static_internal_class;
 ...
 * ?静态内部类的方式创建单例模式
 * 1.静态内部类的创建是不依赖与外部类的 当singleton调用了静态方法getInstance时 静态内部类才会加载且只加载一次 所以内部类中的
 * 静态属性也会跟着初始化一次 且只有一次 下次再调用方法时返回的都只是静态内部类在加载时初始化的实例对象 类装载时线程安全
 *
 * 2.静态内部类不能使用外部类中的任何非静态资源
 *
...
class Singleton {
    ...
    //提供一个私有的静态内部类
    private static final class SingletonInternalClass {
